package üheteiskümmnes.playlist.nestedclasses;

import java.util.ArrayList;

public class Gearbox {

    private ArrayList<Gear> gears;
    private int maxGears;
    private int currentGear;
    private boolean clutchIsIn;

    public Gearbox(int maxGears) {
        this.maxGears = maxGears;
        this.gears = new ArrayList<Gear>();


        //Vabakäik ja käikude lisamine
        Gear neutral = new Gear(0,0);
        this.gears.add(neutral);
        for (int i=1; i<=maxGears; i++){
            addGear(i, i*5.3);
        }

    }

    public void operateClutch (boolean in){
        this.clutchIsIn = in;
    }

    public void addGear ( int number, double ratio){
        if (number>0 && number<=maxGears){
            this.gears.add(new Gear(number, ratio));
        }
    }
    public void changeGear (int newGear){
        if (newGear >= 0 && newGear<=maxGears && this.clutchIsIn){

            this.currentGear = newGear;
            System.out.println(newGear + ". käik sees");
        } else {
            System.out.println("Kraabib");
            this.currentGear = 0;
        }
    }

    public double wheelSpeed (int revs){
        if (clutchIsIn){
            System.out.println("Vali hääl");
            return 0.0;
        }
        else {
            //return revs * gears.get(currentGear).getRatio();
            return gears.get(currentGear).driverSpeed(revs);
        }
    }

    private class Gear {
        private int gearNumber;
        private double ratio;

        public Gear(int gearNumber, double ratio) {
            this.gearNumber = gearNumber;
            this.ratio = ratio;
        }

        public int getGearNumber() {
            return gearNumber;
        }

        public double getRatio() {
            return ratio;
        }
        public double driverSpeed(int revs){
            return revs * this.ratio;
        }
    }

}
